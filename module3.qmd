...
png(filename="Q6plot.png", width=800, height=900)
complete_plot+ plot_layout(ncol = 2, widths = unit(80, "mm"), heights = unit(80, "mm"))
```

```{r}
# show the png I just saved
library(png)
img <- readPNG("Q6plot.png")
plot(grid::grid.raster(img))
```
...
Baltimore_redlining_NDVI <- transformed_baltimore_redlining
neighborhood_mean_NDVI <- pull(terra::extract(BaltimoreNDVI, baltimorePoly, fun = mean), Band08)

neighborhood_median_NDVI <- pull(terra::extract(BaltimoreNDVI, baltimorePoly, fun = median), Band08)
...
Baltimore_redlining_NDVI <- Baltimore_redlining_NDVI |> 
  mutate("NDVI mean" = meanNDVI, "NDVI median" = medianNDVI, "NDVI 2.5% Quantile"= bottomQuantileNDVI,"NDVI 97.5% Quantile"= topQuantileNDVI)
```
